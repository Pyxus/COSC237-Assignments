...
public class OrderedArrayList extends ArrayListClass{
    ...
    // Note For Steve: this method takes advantage of the fact that both list will be sorted before merging.
    public OrderedArrayList merge(OrderedArrayList oaList) {
        int mergedListSize = length + oaList.length;
        OrderedArrayList mergedList = new OrderedArrayList(length + mergedListSize);
        int i = length - 1;
        int j = oaList.length - 1;
        int insertPos = mergedListSize - 1;
        while (insertPos >= 0) {
            int mergeValue;
            if (j < 0 || (i >= 0 && retrieveAt(i) >= oaList.retrieveAt(j))) {
